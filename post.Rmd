---
title: "Of Literary Standards and Logistic Regression: A Replication"
date: January 4, 2015
categories: [dh,theory,sociological-bandwagon,kludgetastic]
output:
    md_document:
        variant: markdown
        pandoc_args: ["--no-wrap"]
...
For the most part, I'm going to omit the R code I'm using, except where it makes a point about the analysis. The full R markdown source for this post is [in this repository](). Here we go...
...
We actually have `r nrow(pruned_counts)` features, a superset of U&S's, because we have allowed for ties at the bottom of the frequency list. If the results are sensitive to this, that's something we'd like to know, so let's continue without discarding those extra features.[^numeric-check]
...
Here is the modeling step.^[Both a `for` loop and assigning to a vector one element at a time, and proud of it.]
...
We don't actually specify the regularization constant lambda in the call to `glmnet`. The documentation for `glmnet` warns us that specifying a single lambda is much slower than computing a "regularization path" (that is, trying a sequence of lambda values) and then doing the prediction for fixed lambda afterwards, but in this case doing the regularization paths takes about 50 times as long. 
...
meta %>%
    ...
    ggplot(aes(year, prob)) +
        ...
        labs(x="publication year",
             ...
             title="Regularized logistic regression predictions of poetry reviewing over time")
```
...
The visual impression of this plot is very similar to U&S's figure. The key features of the plot that U&S emphasize are here: mostly correct classifications (`r
repro_cm["TRUE", "reviewed"] + repro_cm["FALSE", "random"]
` out of `r nrow(feats)`),
...
Changing lambda does not change this list, and it confirms two observations made by U&S: first, four of these "random" volumes are immediately recognizable as high-prestige titles, with the exception of Dobson. Second, they belong to the late end of the period. Now let's consider reviewed volumes with lowest modeled odds:
...
The beautifully available data make it possible to investigate what would be lost and gained by making different choices in the analysis. They also make it possible to build on the work that U&S have done. And


### Searching for historical change
